; NOROM.SYS - DOS Device Driver to Disable ROM at E000
; Tandy 1000 TL/2 ROM Disable Driver
;
; Assemble with: TASM NOROM.ASM
; Link with: TLINK /T NOROM.OBJ
; Rename to: NOROM.SYS
;
; Load in CONFIG.SYS:
;   DEVICE=C:\PATH\TO\NOROM.SYS
;
; This driver sets Port FFEA bit 4 during boot to disable ROM at E000:0000,
; freeing 64KB for DOS UMB allocation via EMM386.
...
DeviceHeader:
    ...
    db 'NOROM   '           ; Device name (8 characters)
...
Init:
    ...
    ; Check if bit 4 is already set
    test al, 10h
    jnz AlreadyDisabled

    ; Set bit 4 (ROM disable)
    or al, 10h
    out dx, al
...
;---------------------------------------------------------------------------
MsgSuccess  db 13, 10
            db 'NOROM: ROM at E000 disabled successfully (Port FFEA bit 4 set)'
            db 13, 10, '$'

MsgAlready  db 13, 10
            db 'NOROM: ROM at E000 already disabled'
            db 13, 10, '$'

MsgFailed   db 13, 10
            db 'NOROM: WARNING - Failed to set Port FFEA bit 4'
            db 13, 10, '$'
